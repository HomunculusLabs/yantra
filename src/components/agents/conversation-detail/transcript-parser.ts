...
function isDiffContentLine(line: string): boolean {
  ...
  if (
    /^(index |new file|deleted file|old mode|new mode|similarity|rename|copy)/.test(
      line
    )
  ) {
    return true;
...
function parseDiffBlock(
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("+++") || line.startsWith("---")) {
      ...
      diffLines.push({ kind: hasHunks ? "plain" : "header", text: line });
    } else {
      break;
    }
    i += 1;
...
function parseCodeBlock(
  ...
  while (i < lines.length) {
    if (lines[i] === "```") {
      ...
      const allStructured =
        nonEmpty.length > 0 && nonEmpty.every((l) => STRUCTURED_RE.test(l));
...
function parseStructuredLine(line: string): Block | null {
  const match = line.match(STRUCTURED_RE);
  if (!match) return null;
