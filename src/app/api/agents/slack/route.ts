...
export async function POST(req: NextRequest) {
  ...
  if ((agent || "human") === "human") {
    ...
    if (mentionedSlugs.length > 0) {
      ...
      if (respondingSlug) {
        markAgentResponding(respondingSlug, channel);
        runQuickResponse(respondingSlug, content, channel)
          .catch(() => {})
          .finally(() => clearAgentResponding(respondingSlug));
      }
    } else {
      ...
      if (targetDept) {
        ...
        if (lead) {
          markAgentResponding(lead.slug, channel);
          runQuickResponse(lead.slug, content, channel)
            .catch(() => {})
            .finally(() => clearAgentResponding(lead.slug));
        }
      }
    }
  }
